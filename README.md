...
MADO はアプリ本体と作業データを分離しています。

| データ | 保存先 | 説明 |
|--------|--------|------|
| アプリ本体 | 任意の場所 | ダウンロード・展開した場所。自由に移動可能 |
| アプリ設定 (`config/settings.yaml`) | アプリ内 `config/` | どの作業フォルダを使うかの記録 |
| エージェント設定 (`agent_profiles.json`) | アプリ内 `config/` | エージェントプロファイル |
| プロジェクトデータ | 作業フォルダ内 | プロジェクト設定・ワークスペース・バックアップ等 |

- **作業フォルダ**は初回起動時にエクスプローラー風UIで指定します。既存フォルダを選ぶだけでOKです。
- 作業フォルダ内に必要なフォルダ・ファイルは自動で作成されます。
- アプリ本体は別の場所に移動しても、作業フォルダのパスさえ正しければデータに影響はありません。
